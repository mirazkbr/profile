...
const App = () => {
  ...
  useEffect(() => {
    // Retrieve cover URL from local storage on component mount
  ...
  const handleCoverChange = (e) => {
    ...
    uploadBytes(storageRefCover, file).then((snapshot) => {
      getDownloadURL(storageRefCover).then((downloadURL) => {
        set(dbRef(db, 'cover/'), {
          img: downloadURL,
        });
        setCoverUrl(downloadURL);

        // Save cover URL to local storage
  ...
  const handleProfileChange = (e) => {
    ...
    uploadBytes(storageRefPro, file).then((snapshot) => {
      getDownloadURL(storageRefPro).then((downloadURL) => {
        set(dbRef(db, 'profile/'), {
          img: downloadURL,
        });
